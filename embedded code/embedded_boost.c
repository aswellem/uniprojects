...
uint8_t rotary_AB(uint8_t rot){
	return (rot & _BV(PB1)) | (rot & _BV(PB0));
}
...
{
	uint16_t cnt =0;
    uint8_t AB, lastAB;  	                                             /* Work in main function was collaborative, with Dan Kim (sk4g18) */
	init_stdio2uart0();
	init_pwm(); 
	init_adc();
	int x = 0;
	DDRB = 0x00;
	PINB = 0xFF;
	lastAB = rotary_AB(PINB);
//	uint16_t MY_SETPOINT_V = 5;
...
*/
		//printf( "SETPOINT: %d    ", MY_SETPOINT_V);
	    printf( "%04d:  ", cnt );
		 if ((v_load() * VOLTDIV > (MY_SETPOINT_V - (MY_SETPOINT_V * 0.05)))&& (v_load() * VOLTDIV < (MY_SETPOINT_V + (MY_SETPOINT_V*0.05)))){ //Taken from Ben Smith (my lab partner)
				pwm_duty(x);
		 }
	
		else if ((v_load() * VOLTDIV) < MY_SETPOINT_V){ 
